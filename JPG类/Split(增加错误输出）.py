...
class ImageSplitterApp:
    ...
    def process_images(self, excel_path, source_folder, output_folder):
        ...
        try:
            ...
            errors = self.run_error_check(group_info)
            ...
            self.log_message("\n" + "="*60)
            if errors:
                self.log_message(f"⚠️ 注意：本次处理共发现 {len(errors)} 个错误！请查看上面的勘误报告。", "ERROR")
                self.log_message("="*60)
                
                # 在GUI中显示错误提示框
                self.root.after(0, lambda: messagebox.showwarning(
                    "勘误警告", 
                    f"处理完成，但发现 {len(errors)} 个错误！\n\n请查看日志中的勘误报告了解详情。\n\n错误日志已保存到输出文件夹。"
                ))
            else:
                self.log_message("✅ 处理完成，未发现任何错误！", "SUCCESS")
                self.log_message("="*60)
            
            self.update_progress(100, f"\n抽卡完成！已保存到: {output_folder}")
            
            if not errors:
                messagebox.showinfo("完成", "抽卡完成！未发现任何错误。")
    ...
    def run_error_check(self, group_info):
        ...
        # 输出详细汇总
        self.log_message("\n" + "="*50)
        self.log_message("勘误汇总报告:")
        self.log_message("="*50)
        
        if errors:
            self.log_message(f"总共发现 {len(errors)} 个错误:", "WARNING")
            self.log_message("")
            ...
            for error_type, type_errors in error_types.items():
                self.log_message(f"\n【{error_type}】({len(type_errors)}个):", "ERROR")
                for error in type_errors:
                    self.log_message(f"  • 分组: {error['group']}")
                    self.log_message(f"    问题: {error['issue']}")
                    self.log_message(f"    详情: {error['details']}")
            
            # 输出简洁的错误列表
            self.log_message("\n" + "-"*50)
            self.log_message("错误清单（简洁版）:", "WARNING")
            for idx, error in enumerate(errors, 1):
                self.log_message(f"{idx}. [{error['type']}] {error['group']}: {error['issue']}")
    ...
    def save_error_log(self, errors):
        """保存错误日志到文件"""
        if not errors:
            return
        ...
        try:
            ...
            if output_folder:
                ...
                error_log_path = os.path.join(output_folder, f"error_log_{timestamp}.txt")
                ...
                with open(error_log_path, 'w', encoding='utf-8') as f:
                    ...
                    for error_type, type_errors in error_types.items():
                        ...
                        f.write("-"*50 + "\n")
                        for error in type_errors:
                            f.write(f"分组: {error['group']}\n")
                            f.write(f"问题: {error['issue']}\n")
                    ...
                    f.write("="*60 + "\n")
                    for idx, error in enumerate(errors, 1):
                        f.write(f"{idx}. [{error['type']}] {error['group']}: {error['issue']}\n")
                
                self.log_message(f"\n错误日志已保存到: {error_log_path}", "INFO")
        except Exception as e:
            self.log_message(f"保存错误日志失败: {e}", "WARNING")
